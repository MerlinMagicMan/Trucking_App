  ...
  return (
    <div className="pf-sidebar">
      <div className="pf-sidebar-header">
        <h2>Planning</h2>
        {lastUpdated && (
          <span style={{ fontSize: '10px', color: '#64748b' }}>{lastUpdated}</span>
        )}
      </div>
      ...
      <div className="pf-sidebar-body">
        ...
        {/* HOS - compact display with edit toggle */}
        <div className="pf-field">
          <div className="pf-hos-state">
            <span>HOS: {formatMin(hos.drive_remaining_min)} / {formatMin(hos.on_duty_remaining_min)}</span>
            <button type="button" onClick={() => setEditHos(!editHos)}>{editHos ? '✓' : '✎'}</button>
          </div>
          {editHos && (
            <div style={{ marginTop: '4px', display: 'flex', flexDirection: 'column', gap: '3px' }}>
              <div className="pf-input-row">
                <input className="pf-input" type="number" value={hos.drive_remaining_min} onChange={e => setHos({ ...hos, drive_remaining_min: parseInt(e.target.value) || 0 })} placeholder="Drive" />
                <input className="pf-input" type="number" value={hos.on_duty_remaining_min} onChange={e => setHos({ ...hos, on_duty_remaining_min: parseInt(e.target.value) || 0 })} placeholder="Duty" />
              </div>
              <input className="pf-input" type="number" value={hos.cycle_remaining_min} onChange={e => setHos({ ...hos, cycle_remaining_min: parseInt(e.target.value) || 0 })} placeholder="Cycle (min)" style={{ maxWidth: '120px' }} />
            </div>
          )}
        </div>
        ...
        <details className="pf-advanced">
          ...
          <div className="pf-advanced-body">
            <input className="pf-input" type="text" value={preferRegions} onChange={e => setPreferRegions(e.target.value)} placeholder="Prefer: TX, OK" />
            <input className="pf-input" type="text" value={avoidRegions} onChange={e => setAvoidRegions(e.target.value)} placeholder="Avoid: NY, NJ" />
            <input className="pf-input" type="number" value={homeTimeDays} onChange={e => setHomeTimeDays(e.target.value)} placeholder="Home-time (days)" />
          </div>
        </details>
        ...
        <details className="pf-advanced">
          ...
          <div className="pf-advanced-body">
            <input className="pf-input" type="number" value={maxDeadhead} onChange={e => setMaxDeadhead(e.target.value)} placeholder="Max deadhead (mi)" />
            <select className="pf-select" value={riskTolerance} onChange={e => setRiskTolerance(e.target.value as 'low' | 'medium' | 'high')}>
        ...
        {csvCount > 0 && (
          <div style={{ fontSize: '10px', color: '#059669', marginTop: '2px' }}>{csvCount} routes uploaded</div>
        )}
      </div>
      ...
      <div className="pf-status-bar">
        ...
        {planCount !== undefined && planCount > 0 && (
          <span className="pf-status-text">{planCount} plans · {loadsAnalyzed || 0} routes</span>
        )}
      </div>
    </div>
  );
